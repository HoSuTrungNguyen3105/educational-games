...
const NAV_ITEMS = (userAuth) => [
  ...
  { key: "profile", icon: "👤", label: "Hồ sơ", path: "/profile", show: !!userAuth?.user },
];
...
const QUICK_MENU_ITEMS = (userAuth) => [
  ...
  { key: "friends", icon: "🔍", label: "Tìm bạn", path: "/find-friends", show: !!userAuth?.user, tint: "from-emerald-400 to-teal-400" },
  { key: "profile", icon: "👤", label: "Hồ sơ", path: "/profile", show: !!userAuth?.user, tint: "from-pink-400 to-rose-400" },
  { key: "teacher", icon: "👨‍🏫", label: "Giáo viên", path: "/admin", show: true, tint: "from-indigo-400 to-violet-400" },
  { key: "login", icon: "🔑", label: "Đăng nhập", action: "login", show: !userAuth?.user, tint: "from-purple-400 to-pink-400" },
];
...
export default function HomeScreen({ onSelectGame, userAuth, onUserLogin, onUserRegister, onUserLogout }) {
  ...
  const handleQuickMenuClick = (item) => {
    ...
    if (item.action === "scroll") return document.getElementById('games-section')?.scrollIntoView({ behavior: 'smooth' });
    if (item.path) return goTo(item.path);
  };

  return (
    <div className="min-h-screen bg-gradient-to-b from-sky-50 via-purple-50/50 to-pink-50 lg:flex">

      {/* ───────── Sidebar (desktop) ───────── */}
      <aside className="hidden lg:flex lg:flex-col lg:w-72 lg:shrink-0 lg:h-screen lg:sticky lg:top-0 bg-white border-r border-purple-100 px-5 py-6">
        <a href="#/" onClick={() => navigate("/")} className="flex items-center gap-2 font-display text-xl text-transparent bg-clip-text bg-gradient-to-r from-purple-600 to-pink-500 mb-6">
          🎪 Lớp Học Vui
        </a>

        {userAuth?.user ? (
          <div className="bg-gradient-to-br from-purple-50 to-pink-50 rounded-2xl p-4 border border-purple-100 mb-4">
            <div className="flex items-center gap-3 mb-3">
              <span className="w-12 h-12 rounded-full bg-gradient-to-br from-purple-400 to-pink-400 text-white flex items-center justify-center text-lg shrink-0">👤</span>
              <div className="min-w-0">
                <p className="text-xs text-gray-500">Xin chào,</p>
                <p className="font-display text-sm text-gray-800 truncate">{userAuth.user.name}</p>
              </div>
            </div>
            ...
            <a
              ...
              href="#/my-coins"
              className="mt-3 flex items-center gap-2 text-sm font-bold text-amber-600 bg-white/70 rounded-xl px-3 py-2 hover:bg-white transition"
            >
              💰 {userCoins.toLocaleString()} coin
            </a>
          </div>
        ) : (
          <div className="bg-gradient-to-br from-purple-50 to-pink-50 rounded-2xl p-4 border border-purple-100 mb-4 text-center">
        ...
        <nav className="flex flex-col gap-1">
          {NAV_ITEMS(userAuth).filter(i => i.show).map(item => (
            <button
              ...
              className={`flex items-center gap-3 text-sm font-semibold px-3 py-2.5 rounded-xl text-left transition ${item.key === "home" ? "bg-purple-50 text-purple-700" : "text-gray-600 hover:bg-purple-50 hover:text-purple-700"
                }`}
            >
        ...
        <div className="mt-auto pt-4 flex flex-col gap-1 border-t border-purple-100">
          <button onClick={() => goTo("/admin")} className="flex items-center gap-3 text-sm font-semibold text-gray-600 px-3 py-2.5 rounded-xl hover:bg-purple-50 hover:text-purple-700 transition text-left">
            <span className="text-lg">👨‍🏫</span> Trang giáo viên
          </button>
          {userAuth?.user && (
            <button onClick={onUserLogout} className="flex items-center gap-3 text-sm font-semibold text-red-500 px-3 py-2.5 rounded-xl hover:bg-red-50 transition text-left">
      ...
      <div className="flex-1 min-w-0 flex flex-col">
          ...
          <div className="px-4 h-16 flex items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-2">
              ...
              {!userAuth?.user ? (
                ...
                <a onClick={() => navigate("/profile")} href="#/profile" className="w-8 h-8 rounded-full bg-gradient-to-br from-purple-400 to-pink-400 text-white flex items-center justify-center text-sm">👤</a>
              )}
              <button
        ...
        <main className="flex-1 w-full max-w-6xl mx-auto px-4 md:px-6 py-8 space-y-10">
          ...
          {/* ───────── Thẻ chào mừng + lưới truy cập nhanh — kiểu "mini app", chỉ hiện trên mobile ───────── */}
          ...
          {subjects.length > 1 && (
            <section>
              <SectionHeader icon="📚" title="Môn học" gradient="from-cyan-500 to-blue-500" />
          ...
          <section id="games-section">
            {games === null ? (
              ...
              <div>
                <SectionHeader icon="🔎" title={`Môn ${activeSubject}`} gradient="from-purple-500 to-indigo-500" />
...
function SubjectTile({ label, icon, active, onClick, classes }) {
  return (
    <button
      onClick={onClick}
  ...
  return (
    ...
    >
      ...
      <div className={`w-full aspect-[4/3] rounded-xl bg-gradient-to-br ${color.grad} flex items-center justify-center mb-2.5`}>
        <StampToken icon={template ? template.icon : "🎲"} ring="#ffffff" size={48} fontSize={22} />
      </div>
