...
export default function ElementsSidebar() {
  ...
  return (
    <div className="w-56 shrink-0 border-r border-ink/10 bg-paper2 flex flex-col">
      ...
      <div className="flex-1 overflow-auto p-3 space-y-1.5">
        {types.map(t => (
          <DraggableElement key={t.type} type={t.type} label={t.label} icon={icons[t.type]} />
        ))}
        {filter !== "game" && <ShapeSubsection />}
      </div>
      <div className="p-3 text-[11px] text-[#B7A987] border-t border-ink/10 leading-relaxed">
...
function ShapeSubsection() {
  ...
  return (
    <div className="pt-2 mt-2 border-t border-ink/10 space-y-1.5">
      ...
      {SHAPE_KINDS.map(s => (
        <button key={s.id} onClick={() => addElement("shape", 120, 120, { kind: s.id })}
