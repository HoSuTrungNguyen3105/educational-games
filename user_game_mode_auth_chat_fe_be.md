...
1. **Ngoài Game**: user có khu vực Chat/Messaging riêng để xem conversation và nhắn tin với nhau.
2. **Trong Game**: khi user vào Game Room, chat vẫn khả dụng nhưng được thu gọn thành **chat bubble ở phía trên khu vực chơi game**. User có thể mở chat mà không rời Game.
...
1. Reconnect socket.
2. Rejoin channels.
3. Sync message/read state nếu cần.
4. Không duplicate message.
...
# Game Mode – Play vs AI / Play vs User

## 1. Tổng quan

Sau khi User đăng nhập và chọn game, cần cho phép chọn hình thức chơi:
...
Hai mode dùng chung Game UI nhưng có flow backend và chat behavior khác nhau.
...
Yêu cầu:
- Không cần matchmaking.
- Không cần chờ User khác.
- Có thể start game ngay.
- Có thể chọn difficulty nếu product hỗ trợ.
- Không tạo user-to-user room chat trong AI mode.

Game session ví dụ:

```ts
type GameMode = "AI" | "USER";
...
Trên mobile, hai lựa chọn cần là card/button lớn, dễ thao tác bằng touch.
...
BE phải đảm bảo User đã cancel thì không thể nhận match từ queue cũ.

---

## 6. Private Match / Invite

Nếu cần chơi với User cụ thể:
...
Invite có thể dùng `inviteCode` hoặc `inviteToken`.

BE phải kiểm tra token hợp lệ, chưa hết hạn và chưa được sử dụng.
...
## 8. Chat Behavior theo Game Mode

### Play vs AI

Nếu không có User khác trong room:

```text
Game
└── No user-to-user room chat
```

Có thể ẩn chat bubble hoặc chỉ hiển thị chat/system UI nếu product có nhu cầu.
...
Chat bubble nằm ở phía trên khu vực chơi game.
...
## 9. Realtime Game Events

Với User-vs-User cần hỗ trợ các event:
...
Trong User-vs-User game cần hiển thị:
...
Presence và Chat có thể dùng chung realtime connection nhưng phải tách state rõ ràng.
...
```http
POST /games/matchmaking/cancel
```

### Match Status

```http
GET /games/matchmaking/status
```
...
Validate mode
       ↓
AI?
├── Yes → Create AI Session
└── No  → Matchmaking / Room
```

Không cho phép User chưa đăng nhập start game nếu product yêu cầu authentication.

BE không được tin tưởng mode, room hoặc opponent do FE tự truyền mà phải validate quyền thực tế.

---

## 13. Acceptance Criteria

- [ ] User có thể chọn Play vs AI.
- [ ] User có thể chọn Play vs User.
- [ ] AI game start được ngay.
- [ ] User game có matchmaking hoặc private room.
- [ ] Matchmaking có searching/matched/error states.
- [ ] User có thể cancel matchmaking.
- [ ] Match thành công đưa hai User vào cùng Game Room.
- [ ] Game Room xác định đúng opponent.
- [ ] Chat bubble chỉ phục vụ User-to-User chat trong User-vs-User mode.
- [ ] Realtime events hoạt động.
- [ ] Disconnect/reconnect opponent được xử lý.
- [ ] FE responsive tốt trên mobile.
- [ ] BE validate authentication, game mode và room membership.
